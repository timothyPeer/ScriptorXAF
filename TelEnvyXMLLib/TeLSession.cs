...
namespace TelEnvyXmlLib
{
    ...
    public partial class TeLSession : Component
    {
        ...
        private void Connect(Directives.SessSeq node)
        {
            ...
            _helper = new VirtualTerminalHelper(terminal) { receiveDataWaitForDataTimeout = receiveInfoTeXTimeout,
        ...
        private int? GrabInt32(int column, int row, int width)
        {

            int buffersize = _helper.Terminal.Screen.Columns;
            if ((column + width) > buffersize) throw new TelEnvyXmlLib.Exceptions.TeLOutputBufferOverFlowException(string.Format("GrabInt32: \n TeXOutputBufferOverFlow Exception\nScreenSize is [{0}], attemped scrap start offset at [{1}] of [{2}] length.", buffersize, column, width));
            string text = _helper.Terminal.Screen.GetRegionText(column, row, width, 1)[0];
            int result;
        ...
        /// <exception cref="TelEnvyXmlLib.Exceptions.TeLOutputBufferOverFlowException"></exception>
        /// <autogeneratedoc />
        /// TODO Edit XML Comment Template for GrabDouble
		private double? GrabDouble(int column, int row, int width)
        {
            int buffersize = _helper.Terminal.Screen.Columns;
            if ((column + width) > buffersize) throw new TelEnvyXmlLib.Exceptions.TeLOutputBufferOverFlowException(string.Format("GrabDouble: \n TeXOutputBufferOverFlow Exception\nScreenSize is [{0}], attemped scrap start offset at [{1}] of [{2}] length.", buffersize, column, width));
        ...
        private string GetPath(string baseDir, string extension)
        {
            ...
            if (!baseDir.EndsWith($@"\")) baseDir = string.Concat(baseDir,@"\");
            string fileName = Path.GetFileNameWithoutExtension(string.Format("{0}_{1}", _moduleName, TransactionGuid.ToString()));
